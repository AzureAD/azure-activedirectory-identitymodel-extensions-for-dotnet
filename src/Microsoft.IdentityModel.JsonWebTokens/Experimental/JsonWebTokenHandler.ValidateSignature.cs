...
#pragma warning restore CA1031 // Do not catch general exception types
            ...
            else
            {
                ...
                key = JwtTokenUtilities.ResolveTokenSigningKey(jwtToken.Kid, jwtToken.X5t, configuration?.SigningKeys)
                    ?? JwtTokenUtilities.ResolveTokenSigningKey(jwtToken.Kid, jwtToken.X5t, validationParameters.SignatureKeys);
            }

            if (key is not null)
                return ValidateSignatureWithKey(jwtToken, validationParameters, key, callContext);

            if (validationParameters.TryAllSignatureKeys)
            ...
            if (string.IsNullOrEmpty(jwtToken.Kid))
            {
                return new SignatureValidationError(
                    new MessageDetail(
                        ...
                        LogHelper.MarkAsNonPII(validationParameters.SignatureKeys.Count),
