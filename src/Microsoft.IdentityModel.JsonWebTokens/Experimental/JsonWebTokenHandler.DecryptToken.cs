...
namespace Microsoft.IdentityModel.JsonWebTokens
{
    public partial class JsonWebTokenHandler : TokenHandler
    {
        ...
        /// <returns>A <see cref="OperationResult{TResult, TError}"/> with OperationResult.Result with the clear text or OperationResult.Error.</returns>
...
#pragma warning restore CA1031 // Do not catch general exception types
            ...
            {
                // TODO - add new TokenDecryptionFailure validation failure type to replace TokenDecryptionFailed KeyWrapFailed
            ...
            CallContext? callContext)
        {
            ...
            if (!string.IsNullOrEmpty(jwtToken.X5t) && validationParameters.TokenDecryptionKeys != null)
            {
                for (int i = 0; i < validationParameters.TokenDecryptionKeys.Count; i++)
                {
                    SecurityKey? key = validationParameters.TokenDecryptionKeys[i];
