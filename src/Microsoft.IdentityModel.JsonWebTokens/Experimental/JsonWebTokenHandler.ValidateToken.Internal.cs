    ...
    public partial class JsonWebTokenHandler : TokenHandler, IResultBasedValidation
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            BaseConfiguration? currentConfiguration =
                GetCurrentConfigurationSync(validationParameters, cancellationToken);

            ValidationResult<ValidatedToken, ValidationError> result = jsonWebToken.IsEncrypted ?
                ValidateJWESync(jsonWebToken, validationParameters, currentConfiguration, callContext, cancellationToken) :
                ValidateJWSSync(jsonWebToken, validationParameters, currentConfiguration, callContext, cancellationToken);
            ...
            if (TokenUtilities.IsRecoverableFailureType(result.Error!.FailureType, (currentConfiguration != null && currentConfiguration.TokenDecryptionKeys.Count > 0)))
            {
                ...
                if (currentConfiguration is not null)
                {
                    ...
                    currentConfiguration = validationParameters.ConfigurationManager.GetBaseConfigurationSync(CancellationToken.None);
                ...
                if (validationParameters.ConfigurationManager.UseLastKnownGoodConfiguration)
                {
                    ...
                    for (int i = 0; i < validConfigurations.Length; i++)
                    {
                        ...
                        {
                            result = jsonWebToken.IsEncrypted ?
                                ValidateJWESync(jsonWebToken, validationParameters, lkgConfiguration, callContext, cancellationToken) :
                                ValidateJWSSync(jsonWebToken, validationParameters, lkgConfiguration, callContext, cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            ValidationResult<ValidatedToken, ValidationError> validationResult =
                ValidateJWSSync(decryptedToken!, validationParameters, configuration, callContext, cancellationToken);
        ...
        private ValidationResult<ValidatedToken, ValidationError> ValidateJWSSync(
        ...
        private static BaseConfiguration? GetCurrentConfigurationSync(ValidationParameters validationParameters, CancellationToken cancellationToken)
        {
            ...
            if (validationParameters.ConfigurationManager is not null)
            {
                try
                {
                    currentConfiguration = validationParameters.ConfigurationManager.GetBaseConfigurationSync(cancellationToken);
                }
