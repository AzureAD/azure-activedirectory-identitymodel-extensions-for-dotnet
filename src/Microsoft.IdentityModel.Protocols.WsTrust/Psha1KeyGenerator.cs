...
namespace Microsoft.IdentityModel.Protocols.WsTrust
{
    ...
    public static class Psha1KeyGenerator
    {
        ...
        /// <summary>
        /// Generates a random key material for issuerEntropy and uses that to create a combined-entropy key.
        /// </summary>
        /// <param name="keySizeInBits">The key size in bits.</param>
        /// <param name="requestorEntropy">The requestor's entropy.</param>
        /// <param name="issuerEntropy">The issuer's entropy (generated by this method).</param>
        ...
        internal static byte[] GenerateSymmetricKey(int keySizeInBits, byte[] requestorEntropy, out byte[] issuerEntropy)
        {
            if (requestorEntropy == null)
                throw LogHelper.LogArgumentNullException(nameof(requestorEntropy));

            int keySizeInBytes = ValidateKeySizeInBytes(keySizeInBits);
            issuerEntropy = new byte[keySizeInBytes];
            _random.GetNonZeroBytes(issuerEntropy);
            return ComputeCombinedKey(issuerEntropy, requestorEntropy, keySizeInBits);
        }
