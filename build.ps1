...
if ($build -eq "YES")
{
    ...
    Write-Host ">>> Start-Process -Wait  -PassThru -NoNewWindow $dotnetexe 'build' -c $buildType -v q /p:SourceLinkCreate=true $root\$slnFile"
    Start-Process -Wait -PassThru -NoNewWindow $dotnetexe "build -c $buildType -v q /p:SourceLinkCreate=true $root\$slnFile"
...
if ($pack -eq "YES")
{
    ...
    foreach($project in $buildConfiguration.SelectNodes("root/projects/src/project"))
    {
        ...
        if ($build -eq "YES")
        {
            Write-Host ">>> Start-Process -Wait -PassThru -NoNewWindow $dotnetexe 'pack' --no-build -c $buildType -o $artifactsRoot -s --include-symbols -v q $root\src\$name\$name.csproj"
            Start-Process  -Wait -PassThru -NoNewWindow $dotnetexe "pack --no-build -c $buildType -o $artifactsRoot -s --include-symbols -v q $root\src\$name\$name.csproj"
        }
        else
        {
            Write-Host ">>> Start-Process -Wait -PassThru -NoNewWindow $dotnetexe 'pack' -c $buildType -o $artifactsRoot -s --include-symbols -v q $root\src\$name\$name.csproj"
            Start-Process -Wait -PassThru -NoNewWindow $dotnetexe "pack -c $buildType -o $artifactsRoot -s --include-symbols -v q $root\src\$name\$name.csproj"
        }
    }
...
if ($runTests -eq "YES")
{
    ...
    foreach ($testProject in $testProjects)
    {
        if ($testProject.test -eq "YES")
        {
            ...
            if ($build -eq "YES")
            {
                Write-Host ">>> Start-Process -Wait -PassThru -NoNewWindow $dotnetexe 'test' --no-build -c $buildType -v q '$name.csproj'"
                $p = Start-Process -Wait -PassThru -NoNewWindow $dotnetexe "test --no-build -c $buildType -v q $name.csproj"
            }
            else
            {
                Write-Host ">>> Start-Process -Wait -PassThru -NoNewWindow $dotnetexe 'test' -c $buildType -v q '$name.csproj'"
                $p = Start-Process -Wait -PassThru -NoNewWindow $dotnetexe "test -c $buildType -v q $name.csproj"
            }
