...
namespace Microsoft.IdentityModel.SampleTests
{
    ...
    public class SampleTokenValidationClassTests
    {
            ...
            Action<ValidationParameters> modifyValidationParameters = null)
        {
            ...
            TokenValidationResult result = await classUnderTest.ValidateTokenShimWithNewPath(token);

            if (!result.IsValid)
                AssertException(expectedInnerExceptionType, expectedInnerExceptionMessagePart, result.Exception);
            else
            {
