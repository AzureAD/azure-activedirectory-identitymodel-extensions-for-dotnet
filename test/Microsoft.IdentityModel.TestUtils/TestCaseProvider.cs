...
namespace Microsoft.IdentityModel.TestUtils
{
    public class TestCaseProvider
    {
        ...
        internal static TheoryData<ValidateTokenTheoryData> GenerateInvalidSignatureTestCases(ITestingTokenHandler tokenHandler)
        {
            ...
            // Token signed with a key that is not available, multiple siging signatureKeys, KeyId present
            ...
            validationParameters = ValidationUtils.CreateValidationParameters(signatureKeys: [Default.AsymmetricSigningKey, Default.AsymmetricSigningKey]);
            ...
            // Token signed with a key that is not available, multiple siging signatureKeys, KeyId NOT present
            securityTokenDescriptor = new SecurityTokenDescriptor
            {
                Audience = Default.Audience,
                Expires = DateTime.UtcNow + TimeSpan.FromDays(1),
            ...
            // Token signed with a key that is not available, multiple siging signatureKeys, KeyId NOT present, TryAllSignatureKeys false
            tokenValidationParameters = ValidationUtils.CreateTokenValidationParameters(keys: [Default.AsymmetricSigningKey, Default.AsymmetricSigningKey]);
            tokenValidationParameters.TryAllIssuerSigningKeys = false;
            validationParameters = ValidationUtils.CreateValidationParameters(signatureKeys: [Default.AsymmetricSigningKey, Default.AsymmetricSigningKey]);
            validationParameters.TryAllSignatureKeys = false;
            ...
            // Token signed with a key that is not available, multiple siging signatureKeys, KeyId NOT present, TryAllSignatureKeys true
            tokenValidationParameters = ValidationUtils.CreateTokenValidationParameters(keys: [Default.AsymmetricSigningKey, Default.AsymmetricSigningKey]);
            tokenValidationParameters.TryAllIssuerSigningKeys = true;
            validationParameters = ValidationUtils.CreateValidationParameters(signatureKeys: [Default.AsymmetricSigningKey, Default.AsymmetricSigningKey]);
            validationParameters.TryAllSignatureKeys = true;
            ...
            // No signatureKeys available, KeyId NOT present
            tokenValidationParameters = ValidationUtils.CreateTokenValidationParameters(keys: []);
            validationParameters = ValidationUtils.CreateValidationParameters(signatureKeys: []);
            theoryData.Add(
