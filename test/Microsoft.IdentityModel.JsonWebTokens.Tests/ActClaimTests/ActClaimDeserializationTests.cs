...
namespace Microsoft.IdentityModel.JsonWebTokens.Tests.ActClaimTests
{
    ...
    public class ActClaimDeserializationTests : IDisposable
    {
        ...
        public ActClaimDeserializationTests(ITestOutputHelper output) => _output = output;

        // Members that legitimately identify an actor inside "act". RFC 8693 section 4.1 shows "sub"
        // (the actor), a nested "act" (progressively earlier actors), and "iss" (used to qualify an
        // actor from a different issuer in the cross-domain example). Lifetime and audience claims are
        // not meaningful inside "act".
        ...
        /// <summary>
        /// A signed JSON object must never contain the same member name twice. A validated ClaimsIdentity
        /// carries BOTH identity.Actor and a retained literal "act" claim, so when it is used as the actor
        /// of a new token the writer must emit the structural "act" only (WriteIdentityClaims skips the
        /// literal "act" claim when identity.Actor is set), producing exactly one nested "act" member.
        /// </summary>
        [Fact]
        ...
        /// <summary>
        /// Duplicate JSON members must resolve identically wherever they appear. The outer payload collapses
        /// duplicate members last-wins (Dictionary indexer assignment in JsonWebToken.PayloadClaimSet), so
        /// CreateActorClaimsIdentityFromJsonElement must do the same - an "act" object and the top-level
        /// payload interpret identical wire bytes the same way - and re-serialization must keep "sub" a
        /// StringOrURI rather than coalescing the duplicates into an RFC 8693-invalid array.
        /// </summary>
        [Fact]
        ...
        /// <summary>
        /// A signed JSON object must never repeat a member name. A validated ClaimsIdentity carries BOTH
        /// identity.Actor and the retained literal "act" claim, so using it as the actor of a new token must
        /// emit the structural "act" only once.
        /// <para>
        /// This test also reports the non-actor claims ("aud"/"exp"/"iat"/"nbf") that a validated identity
        /// carries into "act", but deliberately does not assert on them: the writer serializes the actor
        /// identity as-is and only refrains from injecting its OWN defaults, so stripping them here would be
        /// silent data loss. They appear only because the caller reused a validated identity as an actor.
        /// </para>
        /// </summary>
        [Fact]
