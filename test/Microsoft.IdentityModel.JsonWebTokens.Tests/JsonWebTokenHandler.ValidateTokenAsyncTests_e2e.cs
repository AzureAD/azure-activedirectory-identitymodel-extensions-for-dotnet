...
public class JsonWebTokenHandlerValidateTokenAsyncTestsE2e
{
    ...
    public async Task TestDefaultValidTokenIssuerSigningKeyNotSpecified()
    {
        ...
        Assert.Contains("IDX10527", validationResult.Error.Message);
        // IDX10527: Signature validation failed. The token's kid is: 'JsonWebKeyRsa_2048', but did not match any keys in ValidationParameters or Configuration and TryAllSigningKeys is false. Number of keys in ValidationParameters: '0'. 
    ...
    public async Task TestTokenWithBadSignatureKey()
    {
        ...
        Assert.NotNull(validationResult.Error);
        Assert.IsType<SignatureValidationError>(validationResult.Error);
        Assert.Contains("IDX10526", validationResult.Error.Message);
        // IDX10526: Signature validation failed. The token's kid is missing.
    }
    ...
    public async Task TestTokenWithMissingIssuer()
    {
        ...
        // IDX10211: Unable to validate issuer. The 'issuer' parameter is null or whitespace in ValidationParameters.
    }
    ...
    public async Task TestTokenWithMissingAudience()
    {
        ...
        // IDX10206: Unable to validate audience. The 'audiences' parameter is empty in ValidationParameters.
    }
