...
namespace System.IdentityModel.Test
{
    ...
    public class ExpectedException
    {
        ...
        public static bool ProcessException(ExpectedException exceptionExpected, Exception exception)
        {
            bool retval = false;

            Assert.IsFalse((exceptionExpected == null || exceptionExpected.Thrown == null) && exception != null, string.Format(CultureInfo.InvariantCulture, "DID NOT expect exception, caught: '{0}'", exception));
            Assert.IsFalse(exception.GetType() != exceptionExpected.Thrown, string.Format(CultureInfo.InvariantCulture, "Expected exception of type: '{0}', caught: '{1}'", exceptionExpected.Thrown, exception));
            Assert.IsFalse(!string.IsNullOrWhiteSpace(exceptionExpected.Id) && !exception.Message.Contains(exceptionExpected.Id), string.Format(CultureInfo.InvariantCulture, "Expected Exception.Message to contain: '{0}', caught: '{1}'", exceptionExpected.Id, exception));

            if (exceptionExpected.Inner != null)
            {
                Assert.IsFalse(exception.InnerException == null, string.Format(CultureInfo.InvariantCulture, "Expected InnerException was not found: '{0}' ", exceptionExpected.Inner));
                Assert.IsFalse(exception.InnerException.GetType() != exceptionExpected.Inner.GetType(), string.Format(CultureInfo.InvariantCulture, "Expected InnerException: '{0}', was ex.InnerException: '{1}'", exceptionExpected.Inner, exception.InnerException));
            }
