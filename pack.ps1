...
{
	$name = $project.name;
	Write-Host ">>> Start-Process -wait -NoNewWindow $dotnetexe 'pack' --no-build --no-restore -c $buildType -o $artifactsRoot -v q -s $root\src\$name\$name.csproj"
